...
import { useAddBranchesMutation, useDeleteBranchesMutation, useGetBranchesQuery, useUpdateBranchesMutation } from '../redux/slices/branchesApi'
...
const DashboardBranches = ({ role }) => {
    ...
    const { data: allBranches, isLoading: allBranchesLoading, isError: allBranchesIsError, error: allBranchesError } = useQuery(['Branch'], getAllBranchesApi, {
                        ...
                        <div key={branch._id} className='w-full shadow-[5px_5px_10px_1px_#cdcdcd] p-8 text-dim-black flex gap-4 flex-col sm:flex-row'>
                            ...
                            <div className='flex flex-col w-full'>
                                <div className='flex justify-between'>
                                    ...
                                    <button className='disabled:opacity-50 disabled:hover:opacity-50 cursor-pointer disabled:cursor-not-allowed' onClick={() => handleBranchesDelete(branch._id)} disabled={deleteBranchMutation.isPending}><Trash size={15} className='text-red-600' /></button>
                                </div>
                                <p>{branch.location}</p>
                                <p>{branch.phone}</p>
                                <p className='mt-3'>Staff: {branch.staffs.length}</p>
                                {(role === "Manager") && <button className='cursor-pointer hover:opacity-75 text-blue-600 font-bold rounded-md flex items-center ms-auto text-sm' onClick={() => handleEditBranch(branch)}><SquarePen size={15} className='me-1 inline' />Edit</button>}
                            </div>
                        </div>
                    ))
                }
